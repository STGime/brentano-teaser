<template>
  <div class="min-h-screen bg-blue-50 flex flex-col items-center justify-center py-12 px-4 text-center">
    ...
    <!-- Footer -->
    <footer class="w-full max-w-6xl mx-auto mt-12 pt-8 border-t border-gray-200 text-center text-sm text-gray-500">
      <span>© {{ new Date().getFullYear() }} Brentano Services OÜ. All rights reserved.</span> <!-- Use OÜ -->
      <span class="mx-2">|</span>
      <a @click="showLegalModal" class="text-primary-600 hover:text-primary-800 hover:underline cursor-pointer">
        Imprint, Terms & Conditions
      </a>
    </footer>

    <!-- Legal Info Modal -->
    <!-- LegalModal component defined below is automatically available here -->
    <LegalModal v-if="isLegalModalVisible" @close="hideLegalModal" />

  </div>
</template>

<script setup>
import { ref, reactive, computed, defineComponent } from 'vue'; // Import defineComponent
import LegalModal from '../components/LegalModal.vue'; // Adjust path if needed
...
const handleSignup = async (event) => {
   ...
   if (!validateForm()) {
      signupError.value = "Please correct the errors above.";
      return;
   }
   loading.value = true;
   ...
   try {
       ...
       if (response.ok) {
           ...
           if (responseData.errors && responseData.errors.length > 0) {
               signupError.value = responseData.errors.map(err => err.message || 'Form submission error').join(', ');
               responseData.errors.forEach(err => {
                  if (err.field === 'email') errors.email = err.message;
                  if (err.field === 'name') errors.name = err.message;
               });
           } else {
               signupError.value = 'Form submission failed. Please try again.';
           }
       }
   } catch (error) {
       console.error('Signup error:', error);
       signupError.value = 'An unexpected network error occurred. Please check your connection and try again.';
   } finally {
       loading.value = false;
   }
};
...
console.log("Initial isLegalModalVisible:", isLegalModalVisible.value); // Check initial state
const showLegalModal = () => {
  console.log("showLegalModal function CALLED!"); // <<< Add this log
  isLegalModalVisible.value = true;
  console.log("isLegalModalVisible set to:", isLegalModalVisible.value); // <<< Add this log
};

const hideLegalModal = () => {
  console.log("hideLegalModal function CALLED!"); // Add for completeness
  isLegalModalVisible.value = false;
  console.log("isLegalModalVisible set back to:", isLegalModalVisible.value); // Add for completeness
};
